...
export class SuministrosPage implements OnInit {
  ...
  constructor(private suministrosService: SuministrosService) {}
  ...
  eliminarProducto(index: number) {
    const productoId = this.productos[index].id;
  ...
  registrarEntrega() {
    ...
    // Registrar la entrega en Firebase
    const nuevaEntrega = {
  ...
  eliminarEntrega(index: number) {
    ...
    this.suministrosService.deleteEntrega(entregaId)
      .then(() => {
        ...
        // Recargar las entregas desde Firebase
        this.obtenerEntregas();
      })
      .catch((error) => {
